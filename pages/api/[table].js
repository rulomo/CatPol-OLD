...
const getInfraccions = async (req, res) => {
    const {
        query: { table, page, search },
    } = req;

    
    if (search) {
        const textSearch = search?.split(",").reduce((pre,current)=> `${pre} ${current}`)        
    
        const { data:dataSearch, error } = await supabase
            .rpc("get_infs",{str:`${textSearch}`})
